...
// 判断是否创建表
const handleIsCreatTable = tableName => {
...
const handleSearchPage = (url: string, _body: {}) => {
  // get 获取参数转成数组后再按等号拆成对象值 name=cats&ID=12345&pagesize=10&pageIndex=1 ==> { name: 'Jack', id: 1, pagesize:10, pageIndex: 1  }
  ...
  // 查询条件 WHERE var stmt = this.db.prepare('select userID,username,tokenString from UserToken where username=? and appType=?');
  const selectStmt = db.prepare(`SELECT * FROM cats  WHERE name = ? LIMIT ? OFFSET ?`)
  // 只会返回找到的第一条（结果是对象，非数组）
  // let cats = select_stmt.get(param['name'])
  // 返回找到的所有
  let data = selectStmt.all(param['name'], param['pageSize'], offset)
  console.log('data------', data)

  // 查询条件(所有)
  const selectAllStmt = db.prepare(`SELECT * FROM cats`)
  // 返回所有数据
...
// 根据字段查询并返回所有
const handleGoods = (url: string, _body: {}) => {
  // get 获取参数转成数组后再按等号拆成对象值 name=cats&ID=12345&pagesize=10&pageIndex=1 ==> { name: 'Jack', id: 1, pagesize:10, pageIndex: 1  }
  const param = getQueryStringArgs(url)
  console.log('get参数---', param)
  // 行对象 { id: 1, name: 'Jack', age: 2 }
  ...
  // TODO：扩展分页  LIMIT ? OFFSET ?
  // 查询条件 WHERE var stmt = this.db.prepare('select userID,username,tokenString from UserToken where username=? and appType=?');
  const select_stmt = db.prepare(`SELECT * FROM cats WHERE name = ?`)
  // 只会返回找到的第一条（结果是对象，非数组）
  // let cats = select_stmt.get(param['name'])
  // 返回找到的所有 param['pageSize'], offset
  let cats = select_stmt.all(param['name'])
...
const handleEdit = data => {
  const db = handleIsCreatTable('cats').db

  const updata_stmt = db.prepare('UPDATE cats SET name=@name,age=@age WHERE id=@id')
  // data = { id: 1, name: 'Tom', age: 3 }
  updata_stmt.run(data)
}
...
const handleDelete = data => {
  ...
  // data = { id: 3 }
  delete_stmt.run(data)
}

// 新增(支持批量)
export const handleAdd = (_url: string, body: {}) => {
  console.log('新增接口---', body)
  // 使用函数检查表
  if (handleIsCreatTable('cats').isCreat) {
    console.log(`表 cats 已存在。`)
  } else {
    console.log(`表 cats 不存在。`)
    // 执行创建表
  }
  // 新增接口--- { inputArr: { name: 'ganxiaobo1', age: 10 } }
  // SELECT name FROM sqlite_master WHERE type='table' AND name='cats'
  // 表 cats 已存在。

  // _body 传入数组  inputArr: [{a:1},{b:2}]
  // const addArr = [
  //   { name: 'ganxiaobo1', age: 10 },
  ...
  // const db = require('better-sqlite3')('../database.db', { verbose: console.log })
  // //创建插入语句
  // const insertStmt = db.prepare('INSERT INTO cats (name, age) VALUES (@name, @age)')
  // //插入一条记录
  // // insertStmt.run({ name: 'Jack', age: 2 })
  // //创建批量插入过程
...
// 查询表所有
export const handleSearchAll = (url: string, _body: {}) => {
  const param = getQueryStringArgs(url)
  console.log('get参数---', param)
  // { id: 1, name: 'Jack', age: 2 }
...
// 创建
// const handleCreatTable = (_url: string, _body: {}) => {
...
// INTEGER PRIMARY KEY AUTOINCREMENT代表主键自增
// const create_table_cats = `CREATE TABLE users
//      (
...
// db.exec(create_table_cats) //执行sql命令

// // 新建一个查询语句对象：
// const stmt = db.prepare('SELECT * FROM users ')

// // 使用查询：
// const row = stmt.all() //返回全部记录
// //var row=stmt.get()//返回第一条记录
// console.log(row)
//
// const Database = require('better-sqlite3')
// const db = new Database('temp.sqlite', { verbose: console.log })
...
//更改数据
const updata_stmt = db.prepare('UPDATE cats SET name =@name ,age=@age WHERE id=@id')
var a= updata_stmt.run({id:1,name:"Tom",age:3})
console.log(a)
